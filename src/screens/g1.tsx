...
import { Button, Input, View, Box, Text, HStack, Center, Divider, Container, Heading, Icon} from "native-base";
...
function TaskListScreen({ navigation }: TaskListScreenProps) {
    return (
        <View>
          <Box safeArea p="9" w="90%" maxW="290" py="8"></Box>
            <Center>
            <HStack>

            <Button mt="0" style = {{marginRight: 7, backgroundColor:'#154c79'}} onPress={() => navigation.navigate('AddClass')}>
            ...
            <input type = "date" className="Date" style = {{borderColor: "#154c79", backgroundColor: "#F2F2F2", borderRadius: 6, textAlign:"center"}} ></input>
            {/* When the date is adjusted on the date thing the table should then be filled with the items from that day */}


            <Button mt="0" style = {{backgroundColor:'#154c79', marginLeft: 7}} onPress={() => navigation.navigate('AddClass')}>✔
            </Button>
