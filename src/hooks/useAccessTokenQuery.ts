...
export const useAccessTokenQuery = () => {
    ...
    return useQuery<AccessTokenResponse>('accessTokenResponse', accessTokenRequest, {
        enabled: queryClient.getQueryData<CSRFTokenResponse>('csrfToken')?.status === "success" 
            && queryClient.getQueryData<LoginResponse>('loginResponse')?.status === "success",
        cacheTime: 30 * 60 * 1000, 
        initialData: () => useQueryClient().getQueryData<LoginResponse>('loginResponse')
    });
}
